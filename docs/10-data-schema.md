...
> **Ramsey publishes *partial* structure in its API** — `basement_present`, `stories`, `bedrooms`,
> `garage_sf`, `exterior_wall`, `home_style`, and `effective_year_built` (see [docs/03](03-data-sources.md))
> — but **no grade / condition / CDU and no ABSF ÷ finished-basement split** (those are Beacon-only), and
> comps/sales carry no `sale_code`. The legacy `structure` object stays null for Ramsey (it's the Mpls
> shape). The missing pieces are a limit of the **county API**, not of the workflow: the
> **CAMA-error-correction** and **cost-to-cure** approaches (which need grade, condition, or the
> above-grade/basement split) get that from the **Beacon parse** (`scripts/parse_beacon` → `beacon.json`,
> with `contributory_basement_sf`) and the **agent-driven enrichment step**, not from `collected_data.json`. When a condition/CAMA argument is in play, run the
...
| `land_regression` | obj/null | Independent cross-check on the land line — a regression of the county's own assessed land on lot size, **single assessment year only** (filtered by `emv_year` to the subject's year; never mixes 2025/2026 — a 2026 subject against a 2025-weighted line looks rich when it isn't). Carries `value_fit`/`psf_fit` (`slope`/`intercept`/`r2`), the robust **`indicated_land_value`** and the **`indicated_land_range`** spanning both functional forms, the subject's **`position`** (`within_range` → land confirmed; `above` → assessed rich for its size; `below` → conservative), **`wiggle_room`** (the defensible reduction toward the lower bound, 0 when on/below), **`subject_lot_extrapolated`** (true when the subject's lot is outside the comp lot range → read the range, not a point), and a **`chart`** (land $/SF vs lot size + the trend line, ready for the scatter renderer). `applicable:false` with a note when there are too few same-vintage comps (don't pad it by mixing years). |
| `tax_economics` | obj | `etr`, `etr_proxy_source` (`prior_year_tax` / `county_default`), `savings_per_10k_reduction`, and an **`illustrative_savings`** (likely-reduction × ETR at a default reduction assumption — illustrative only, **not** a forecast or a concluded number) so the worth-it threshold can be applied even when Ramsey carries `total_tax=null`. Also carries a **`worth_it_gate`** (`flag` ∈ `pass`/`borderline`/`fail`/`not_yet_sized`/`n/a — no supportable reduction`) sized off the implied reduction — **informational only; it does NOT change the `verdict`.** The headline is **`annual_client_savings`** (= implied reduction × ETR, the recurring savings to the homeowner) tested against **`min_annual_client_savings`** (the ~$1,000/yr floor); it also carries `min_reduction_to_clear_floor`, `illustrative_reduction`, `illustrative_reduction_source`, `illustrative_year1_fee`, `contingency_pct_assumed`, and `hold_years_assumed`. The test is the **client savings**, not a fee-vs-cost break-even — there is no fixed cost-to-pursue. The implied reduction is sized off a **defensible (governing/market) basis only** — sales convergence or `sales_comparison_indicated` below EMV — **never the equalization median gap** (methodology forbids equalizing to the median, and the script relabels it `median_gap_directional_not_a_basis`), so the gate cannot be sized off a phantom sub-EMV median artifact on a fairly-assessed parcel. The floor and `contingency_pct_assumed` are **illustrative placeholders, not calibrated doctrine** — the worth-it call is made downstream ([run-appeal-review.md](../prompts/run-appeal-review.md) Step 3) with the real ETR (confirm it from the tax statement when the gate is near the floor). |
